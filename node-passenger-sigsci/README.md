# Node Passenger + Fastly Next-Gen WAF (sigsci) Base Image

This is a drop-in variant of [`instructure/node-passenger`](../node-passenger) with the
...
Everything from the base `node-passenger` image still applies — all of the `PASSENGER_*`,
...
[ruby-passenger README](../ruby-passenger/README.md) for the shared Passenger/nginx options.

## Quick start

Change your `FROM` from `instructure/node-passenger:<version>` to
`instructure/node-passenger-sigsci:<version>`. Nothing else changes. Provide your Fastly
...
the `generate:node-passenger-sigsci` Rake task. Edit the template files, run the task, and
